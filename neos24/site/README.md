...
| `lib/` | Gemeinsame Helfer für `intern/` und `konto/` (Maskierung, CSRF, Formate), Kundenkonten und Firmen, Versand-Kern (`versand.php`), Rechnungsprüfung (`rechnungspruefung.php`), Einkaufspreis-Import, Tabellen- (CSV/XLSX) und PDF-Leser ohne Abhängigkeiten, Carrier-Schnittstelle (Stubs), NEOS-Label und Sammelrechnungen als PDF (FPDF vendored) |
