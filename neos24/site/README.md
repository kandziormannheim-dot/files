...
| Datei | Inhalt |
|---|---|
| `index.html` | Die komplette deutsche Seite, semantisch, `lang="de"`, mit Meta/OG/JSON-LD — Versand **weltweit** in drei Kategorien: Brief / Dokumente, Paket (beide buchbar mit eigenen Preisen), Palette (nur auf Anfrage) |
...
| `assets/checkout.js` | Privatkunden-Checkout: Kategorie (Brief / Paket → Gewichtsklassen der Kategorie, Palette → Anfrageformular), Zollhinweis für Ziele außerhalb der EU, Preissumme, Validierung, Bestellung anlegen, Revolut-Popup, Statusabfrage; Preistabelle mit „ab“-Preis je Kategorie — siehe `api/revolut/README.md` |
...
| `api/anfrage.php` | Kontaktformular → Anfrage in der Datenbank (Dashboard → Kunden & Anfragen) + Benachrichtigung ans Postfach; JSON per fetch oder klassisches Formular ohne JS. `typ=palette` legt eine **Palettenanfrage** mit Eckdaten (Anzahl, Art, Gewicht, Abholung, Ziel) an (`palettenanfrageAnlegen()` in `lib/versand.php`, auch vom Portal genutzt) |
| `intern/` | Internes Dashboard mit Rollen und Rechtematrix: Übersicht, Bestellungen, Preise & Zielländer, Routingmatrix, Kunden & Anfragen (Firmen, Privatkunden), Rechnungen, Statistiken & Berichte (Kennzahlen, Diagramme, Pivot, Export), Benutzer & Rollen — siehe `intern/README.md` |
| `konto/` | Kundenportal für Privat- und Geschäftskunden: Sendungen mit Carrier-Vergleich und Zusatzleistungen anlegen (Revolut, Guthaben oder Rechnung), Labels als PDF, Tracking (auch öffentlich), Benutzergruppen mit Rechten je Bereich für Mitarbeiter-Subaccounts, Adressbuch je Benutzer mit Firmenfreigabe, Paketvorlagen, Sendungsimport aus CSV/XLSX, Guthaben, Retouren, Reklamationen, Belegarchiv mit PDF-Download — jeder nur seine eigenen Daten; Login per Passwort oder E-Mail-Link — siehe `konto/README.md` |
| `lib/` | Gemeinsame Helfer für `intern/` und `konto/` (Maskierung, CSRF, Formate), Kundenkonten und Firmen, Versand-Kern (`versand.php`), Kundenpreislisten (`preislisten.php`), Rechnungsprüfung mit automatischer Nachberechnung (`rechnungspruefung.php`), Nachweis-PDF (`nachberechnung_pdf.php`), Lexware Office (`lexware.php`), IMAP-Postfach (`postfach.php`), Einkaufspreis-Import, Belegarchiv (`belege.php`), Sendungsimport (`import.php`), Statistiken (`statistik.php`), Tabellen- (CSV/XLSX) Leser und XLSX-Schreiber sowie PDF-Leser ohne Abhängigkeiten, Carrier-Schnittstelle (Stubs), NEOS-Label und Sammelrechnungen als PDF (FPDF vendored) |
