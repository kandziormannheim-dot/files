...
if ($pfad === '/bestellungen') {
    ...
    $seite = seiteLesen();
    $st = $db->prepare('SELECT * FROM bestellungen' . $sql . ' ORDER BY id DESC LIMIT 50 OFFSET ' . (($seite - 1) * 50));
    $st->execute($werte);
...
if ($pfad === '/kunden') {
    ...
    $reiter = ($_GET['reiter'] ?? 'anfragen') === 'kunden' ? 'kunden' : 'anfragen';
    $status = saeubern($_GET['status'] ?? '', 20);
    $q = saeubern($_GET['q'] ?? '', 100);
    $seite = seiteLesen();
    if ($reiter === 'anfragen') {
        ...
        $sql = $q !== '' ? ' WHERE email LIKE ?' : '';
        $werte = $q !== '' ? ['%' . $q . '%'] : [];
        $st = $db->prepare('SELECT COUNT(DISTINCT email) FROM bestellungen' . $sql);
        $st->execute($werte);
        $gesamt = (int) $st->fetchColumn();
        $st = $db->prepare("SELECT email, COUNT(*) AS anzahl, SUM(CASE WHEN status = 'bezahlt' THEN 1 ELSE 0 END) AS bezahlt, COALESCE(SUM(CASE WHEN status = 'bezahlt' THEN betrag_cent ELSE 0 END),0) AS umsatz, MAX(erstellt) AS letzte, MAX(sprache) AS sprache FROM bestellungen" . $sql . ' GROUP BY email ORDER BY letzte DESC LIMIT 50 OFFSET ' . (($seite - 1) * 50));
        $st->execute($werte);
        $zeilen = $st->fetchAll();
    }
