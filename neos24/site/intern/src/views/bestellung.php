...
<div class="spalten spalten--2-1">
  <div>
    <div class="karte">
      ...
      <p class="hinweis" style="margin-top:.75rem">Gewichtsnachberechnung zu <a class="mono" href="<?= e(url('bestellungen/' . ($nbGrund['original'] ?? ''))) ?>"><?= e($nbGrund['original'] ?? '') ?></a>: gewogen <?= e(number_format((int) ($nbGrund['gewicht_gramm'] ?? 0) / 1000, 2, ',', '')) ?> kg, gebucht <?= e($nbGrund['gk_bestellt'] ?? '') ?>, tatsächlich <?= e($nbGrund['gk_ist'] ?? '') ?><?= isset($nbGrund['verkauf_ist']) ? ' · Listenpreis ' . e(euro((int) $nbGrund['verkauf_ist'])) . ' − ' . e(euro((int) $nbGrund['verkauf_bestellt'])) : '' ?><?= (int) ($nbGrund['gebuehr'] ?? 0) > 0 ? ' + Carrier-Gebühr ' . e(euro((int) $nbGrund['gebuehr'])) : '' ?><?= !empty($nbGrund['lieferantenrechnung']) ? ' · Lieferantenrechnung ' . e($nbGrund['lieferantenrechnung']) . ' (intern)' : '' ?>.
        <?php if (nachberechnungNachweisPfad($b) !== '' && is_file(nachberechnungNachweisPfad($b))) { ?><a class="knopf knopf--leise knopf--klein" href="<?= e(url('bestellungen/' . $b['ext_ref'] . '/nachweis.pdf')) ?>" target="_blank" rel="noopener">Nachweis (PDF)</a><?php } ?>
      </p>
      <?php } ?>
      <?php if ((int) ($b['preisliste_id'] ?? 0) > 0) { $pl = preislisteLaden((int) $b['preisliste_id']); ?><p class="leise" style="margin-top:.5rem">Preisliste: <a href="<?= e(url('kunden/preisliste/' . $b['preisliste_id'])) ?>"><?= e($pl['name'] ?? '#' . $b['preisliste_id']) ?></a></p><?php } ?>
      <?php if ((int) ($b['volumen_gramm'] ?? 0) > (int) $b['gewicht_gramm']) { ?><p class="leise" style="margin-top:.5rem">Volumengewicht <?= e(number_format((int) $b['volumen_gramm'] / 1000, 2, ',', '')) ?> kg war maßgeblich für die Klasse.</p><?php } ?>
