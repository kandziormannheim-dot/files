...
<div class="kpi-raster">
  ...
  <div class="kpi kpi--gelb"><span class="kpi-name">Abholungen offen</span><strong class="kpi-wert"><?= (int) $k['abholungenOffen'] ?></strong><span class="kpi-zusatz">beauftragt, noch nicht übergeben<?= darf('bestellungen') ? ' · <a href="' . e(url('bestellungen', ['abholung' => '1'])) . '">Liste</a>' : '' ?></span></div>
  <div class="kpi kpi--magenta"><span class="kpi-name">Offene Reklamationen</span><strong class="kpi-wert"><?= (int) $k['reklamationenOffen'] ?></strong><span class="kpi-zusatz">Kundenguthaben gesamt <?= e(euro((int) $k['guthabenGesamt'])) ?><?= darf('reklamationen') ? ' · <a href="' . e(url('reklamationen')) . '">öffnen</a>' : '' ?></span></div>
  <div class="kpi kpi--cyan"><span class="kpi-name">Lieferantenrechnungen zu prüfen</span><strong class="kpi-wert"><?= (int) $k['lieferantenOffen'] ?></strong><span class="kpi-zusatz">Nachberechnung wartet auf Freigabe <?= e(euro((int) $k['nachberechnungOffen'])) ?> netto<?= darf('rechnungspruefung') ? ' · <a href="' . e(url('rechnungspruefung')) . '">öffnen</a>' : '' ?></span></div>
  <?php if (lexwareAktiv() || (int) $k['lexwareFehler'] > 0) { ?><div class="kpi <?= (int) $k['lexwareFehler'] > 0 ? 'kpi--coral' : 'kpi--gelb' ?>"><span class="kpi-name">Lexware-Übergaben</span><strong class="kpi-wert"><?= (int) $k['lexwareFehler'] ?></strong><span class="kpi-zusatz">fehlgeschlagen · <?= (int) $k['lexwareOffen'] ?> offen<?= darf('rechnungen') ? ' · <a href="' . e(url('rechnungen')) . '">Warteschlange</a>' : '' ?></span></div><?php } ?>
  <?php if ($k['syncSysteme'] !== [] || (int) $k['syncFehler'] > 0 || (int) $k['syncKonflikte'] > 0) { ?><div class="kpi <?= (int) $k['syncFehler'] > 0 || (int) $k['syncKonflikte'] > 0 ? 'kpi--coral' : 'kpi--cyan' ?>"><span class="kpi-name">Synchronisation</span><strong class="kpi-wert"><?= (int) $k['syncFehler'] + (int) $k['syncKonflikte'] ?></strong><span class="kpi-zusatz"><?= (int) $k['syncFehler'] ?> Fehler · <?= (int) $k['syncKonflikte'] ?> Konflikte · <?= (int) $k['syncOffen'] ?> offen<?= darf('sync') ? ' · <a href="' . e(url('sync')) . '">öffnen</a>' : '' ?></span></div><?php } ?>
